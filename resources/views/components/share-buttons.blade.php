...
<div {{ $attributes->merge(['class' => 'share-buttons ' . $class]) }} x-data="{ copied: false }">
    <div class="flex flex-wrap items-center gap-2">
        <span class="text-sm font-medium text-gray-700 mr-2">Bagikan:</span>
        
        @foreach(['facebook', 'twitter', 'whatsapp', 'telegram', 'linkedin'] as $platform)
            <a href="{{ $shareLinks[$platform]['url'] }}"
               target="_blank"
               rel="noopener noreferrer"
               class="inline-flex items-center justify-center w-9 h-9 rounded-full transition-all duration-200 transform hover:scale-110"
               style="background-color: {{ $shareLinks[$platform]['bgColor'] }}; color: #ffffff;"
               onmouseover="this.style.backgroundColor='{{ $shareLinks[$platform]['hoverColor'] }}'"
               onmouseout="this.style.backgroundColor='{{ $shareLinks[$platform]['bgColor'] }}'"
               title="Bagikan ke {{ $shareLinks[$platform]['label'] }}"
               aria-label="Bagikan ke {{ $shareLinks[$platform]['label'] }}">
        ...
        <button type="button"
                ...
                class="inline-flex items-center justify-center w-9 h-9 rounded-full transition-all duration-200 transform hover:scale-110 cursor-pointer"
                :style="copied ? 'background-color: #16a34a; color: #ffffff;' : 'background-color: {{ $shareLinks['copy']['bgColor'] }}; color: #ffffff;'"
                title="Salin Link"
                aria-label="Salin Link">
            <template x-if="!copied">
                {!! $shareLinks['copy']['icon'] !!}
            </template>
            <template x-if="copied">
