...
@section('title', 'Berita - ' . ($settings['site_name'] ?? config('app.name')))

@section('content')
<x-breadcrumb :items="[['label' => 'Berita']]" />

<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    {{-- Header --}}
    <div class="text-center mb-12">
        <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Berita & Artikel</h1>
        <p class="text-lg text-gray-600 max-w-2xl mx-auto">
            Informasi dan berita terbaru dari {{ $settings['site_name'] ?? 'Universitas Bumigora' }}
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-200">
                ...
                <div class="flex items-center space-x-4">
                    <select onchange="window.location.href=this.value" 
                        ...
                        <option value="{{ request()->fullUrlWithQuery(['sort' => 'latest']) }}" {{ request('sort', 'latest') == 'latest' ? 'selected' : '' }}>Terbaru</option>
                        <option value="{{ request()->fullUrlWithQuery(['sort' => 'oldest']) }}" {{ request('sort') == 'oldest' ? 'selected' : '' }}>Terlama</option>
                        <option value="{{ request()->fullUrlWithQuery(['sort' => 'popular']) }}" {{ request('sort') == 'popular' ? 'selected' : '' }}>Populer</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-8">
                <x-article-card :article="$featuredArticle" :featured="true" :currentUnitType="$unitType ?? null" :currentUnitId="$unitId ?? null" :fakultas="$fakultas ?? null" />
            </div>
            @endif
